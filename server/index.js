...
authEmailHelpers = registerAuthEmailRoutes(app, db, { onboardingUrl: N8N_ONBOARDING_URL })
...
app.post('/api/check-account', async (req, res) => {
  ...
  const { authProvider, providerUserId, email } = req.body || {}
  if (authProvider === 'email' && authEmailHelpers) {
    try {
      const row = await authEmailHelpers.lookupEmailUser(providerUserId, email)
      if (row) {
        const profile = authEmailHelpers.profileFromRow(row)
        return res.json({
          exists: true,
          step: 2,
      ...
      console.error('[check-account/email]', err.message)
    }
  }
...
// SECURITY: only n8n URLs are allowed — otherwise this endpoint would let
// anyone use the server as an open proxy (SSRF).
app.post('/api/resume', async (req, res) => {
  const { resumeUrl } = req.query
  if (!resumeUrl) return res.status(400).json({ error: 'Missing resumeUrl' })
  let host
  try { host = new URL(resumeUrl).host } catch { host = null }
  if (host !== 'buzobue.app.n8n.cloud') {
    return res.status(400).json({ error: 'Invalid resumeUrl' })
  }
