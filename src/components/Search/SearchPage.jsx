...
import AnalysisOverlay, { SEARCH_MESSAGES } from '../UI/AnalysisOverlay.jsx'
import { getUserPhotoSrc, getUserInitials, resizePhotoFile } from '../../utils/userPhoto.js'
...
export default function SearchPage({ user, onBack, onSignOut, onUpdateUser, onOpenProfile, onOpenCart }) {
  ...
  async function performSearch() {
    ...
    const quota = await checkAnalysisQuota(user)
    if (!quota.allowed) {
      setError(t('quota.blocked'))
      showNativePaywall()
      return
    }
      ...
      await sendLinkStep(trimmed, user.id, searchId, {
        onUpdate: (updated) => {
          ...
          if (updated.status === 'completed') {
            recordCompletedAnalysis(user, 'search')
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <button
              ...
              style={{
                ...
                background: getUserPhotoSrc(user) ? 'transparent' : `linear-gradient(135deg, ${S.warm}, #8B6545)`,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 13, color: 'white', fontWeight: 600,
                border: getUserPhotoSrc(user) ? `1.5px solid ${S.border}` : 'none',
                cursor: 'pointer', overflow: 'hidden',
              }}
            >
              {getUserPhotoSrc(user) ? (
                <img src={getUserPhotoSrc(user)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : getUserInitials(user).slice(0, 1)}
            </button>
          </div>
        </div>
      </div>
...
function ProfileEditSheet({ user, onClose, onSave }) {
  ...
  async function handleFile(e) {
    ...
    if (!file) return
    try {
      // Downscale + bake EXIF orientation before upload
      setPhotoBase64(await resizePhotoFile(file))
    } catch { /* invalid image — keep previous photo */ }
  }
